...
import {
  ...
  MessageSquare, Key, Hash, FileText, Settings, LayoutDashboard
} from "lucide-react";
...
function TailorDashboard() {
  ...
  // Editable fields (only what shop manager can change)
  ...
  const fetchShopData = useCallback(async (shopId: string) => {
    ...
    if (!shopData) { navigate({ to: '/tailor/login' }); return; }
  ...
  useEffect(() => {
    ...
    if (auth !== 'true' || !shopId) { navigate({ to: '/tailor/login' }); return; }
        ...
        <Tabs value={activeTab} onValueChange={setActiveTab}>
          ...
          <TabsContent value="overview" className="space-y-6 mt-6">
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              ...
              <Card className="shadow-sm">
                <CardContent className="p-6 flex items-center gap-4">
                  <div className="w-12 h-12 rounded-xl bg-blue-100 flex items-center justify-center"><MessageSquare className="w-6 h-6 text-blue-600" /></div>
                  <div>
                    ...
                    <div className="flex flex-col md:flex-row items-center gap-8">
                      ...
                      <div className="flex-1 w-full space-y-4">
                        ...
                        <div className="space-y-2">
                          ...
                          <Input value={shop.manager_phone} disabled className="h-12 rounded-xl bg-muted/50" />
              ...
              <div className="space-y-8">
                <Card className="shadow-xl border-none overflow-hidden rounded-[2rem]">
                  <CardHeader className="bg-blue-50/50 pb-6 border-b border-blue-100">
                    <CardTitle className="flex items-center gap-2 text-blue-800"><Share2 className="w-5 h-5" />شبکه‌های اجتماعی</CardTitle>
                  </CardHeader>
                  <CardContent className="p-6 space-y-4">
                    <div className="space-y-2"><Label className="flex items-center gap-2 text-pink-600 font-bold"><Instagram className="w-4 h-4" />اینستاگرام</Label><Input value={editInstagram} onChange={(e) => setEditInstagram(e.target.value)} dir="ltr" className="rounded-xl h-11" /></div>
                    <div className="space-y-2"><Label className="flex items-center gap-2 text-blue-400 font-bold"><Send className="w-4 h-4" />تلگرام</Label><Input value={editTelegram} onChange={(e) => setEditTelegram(e.target.value)} dir="ltr" className="rounded-xl h-11" /></div>
                    <div className="space-y-2"><Label className="flex items-center gap-2 text-green-600 font-bold"><MessageCircle className="w-4 h-4" />واتس‌اپ</Label><Input value={editWhatsapp} onChange={(e) => setEditWhatsapp(e.target.value)} dir="ltr" className="rounded-xl h-11" /></div>
                  </CardContent>
                </Card>

                <Card className="shadow-xl border-none overflow-hidden rounded-[2rem]">
                  <CardHeader className="bg-orange-50/50 pb-6 border-b border-orange-100">
                    <CardTitle className="flex items-center gap-2 text-orange-800">شبکه‌های ایرانی</CardTitle>
                  </CardHeader>
                  <CardContent className="p-6 space-y-4">
                  ...
                  <Card key={branch.id} className="shadow-sm">
                    ...
                    <CardContent className="space-y-2 text-sm">
                      ...
                      <div className="flex flex-wrap gap-3">
                        ...
                        {branch.whatsapp && <Badge variant="outline" className="gap-1 text-green-600"><MessageCircle className="w-3 h-3" />{branch.whatsapp}</Badge>}
                      </div>
                    </CardContent>
                  </Card>
                ))}
